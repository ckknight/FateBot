...
function real($staropt$star, min, max) {
  var inclusive = $staropt$star ? $staropt$star[0] : /* false */0;
  var makeFloat = RandomJs.real(min, max, Js_boolean.to_js_boolean(inclusive));
  return (function (t) {
      return makeFloat(t);
    });
}
...
function pick(start, finish, array, engine) {
  return Js_primitive.undefined_to_opt(RandomJs.pick(engine, array, Js_undefined.from_opt(start), Js_undefined.from_opt(finish)));
}

function picker(start, finish, array) {
  var makePick = RandomJs.picker(array, Js_undefined.from_opt(start), Js_undefined.from_opt(finish));
  return (function (t) {
...
function hex(uppercase) {
  var makeString = RandomJs.hex(Js_boolean.to_js_boolean(uppercase));
  return (function (length, t) {
      return makeString(t, length);
    });
}
