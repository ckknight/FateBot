...
var MakeClient              = require("giphy-api");
...
function parseMatch(commandName, args) {
  switch (commandName) {
    ...
    case "ping" : 
        return /* Some */[/* Ping */0];
    case "poll" : 
        var match_ = args.match(new RegExp("^(\\d+)(?:-(\\d+))?\\s+"));
        if (match_ !== null) {
          if (Boolean(Caml_array.caml_array_get(match_, 2))) {
            return /* Some */[/* PollRange */Block.__(3, [
                        Utils.$$parseInt(1, Caml_array.caml_array_get(match_, 1)),
                        Utils.$$parseInt(10, Caml_array.caml_array_get(match_, 2))
                      ])];
          } else {
            return /* Some */[/* PollRange */Block.__(3, [
                        1,
                        Utils.$$parseInt(10, Caml_array.caml_array_get(match_, 1))
                      ])];
          }
        } else {
          return /* Some */[/* Poll */4];
